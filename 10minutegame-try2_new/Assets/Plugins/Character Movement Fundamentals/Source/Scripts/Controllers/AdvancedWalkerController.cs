...
namespace CMF
{
    ...
    public class AdvancedWalkerController : Controller {
        ...
        public void DoJumpMoblie()
        {
            jumpInputIsLocked = false;
            jumpMoblie = true;
        }

        public void OnJumpInertia()
        {
            OnGroundContactLost();
            OnJumpStart();
        ...
        void ControllerUpdate()
		{
            ...
            Transform lastTr = mover.GetGroundTransform();
            if (lastTr != null && !lastTr.CompareTag("DeadTrigger"))
                lastTransform = lastTr;
            ...
            limitedInputX = (inputX * 3) * (IsGrounded() ? 1 : 0);
            limitedInputY = (inputY * 3) * (IsGrounded() ? 1 : 0);
		...
		protected virtual Vector3 CalculateMovementDirection()
		{
            ...
            if (mobile_Input && playerRespawn.isMobile)
            {
                ...
                inputX = mobile_Input.MoveInput().x;
                inputY = mobile_Input.MoveInput().z;

            }
            else
            {
                //Get input values;
                inputX = characterInput.GetHorizontalMovementInput();
                inputY = characterInput.GetVerticalMovementInput();
            }
			...
			{
				_velocity += tr.right * inputX;
				_velocity += tr.forward * inputY;
			}
			else
			{
				//If a camera transform has been assigned, use the assigned transform's axes for movement direction;
				//Project movement direction so movement stays parallel to the ground;
				_velocity += Vector3.ProjectOnPlane(cameraTransform.right, tr.up).normalized * inputX;
				_velocity += Vector3.ProjectOnPlane(cameraTransform.forward, tr.up).normalized * inputY;
			}
		...
		ControllerState DetermineControllerState()
		{
			...
			if(currentControllerState == ControllerState.Grounded)
			{
				...
				if(!mover.IsGrounded()){
                    ...
                    if (disableFalling == false)
                    {
                        disableFalling = true;
                        countPreFall = Time.fixedTime + preFallTime;
                    }

                    if (Time.fixedTime >= countPreFall || Physics.Raycast(transform.position, Vector3.down, 2.5f))
                    {
