...
namespace CMF
{
	...
	public class CameraController : MonoBehaviour
    {
        ...
        void Awake () {
            ...
            //advancedWalkerController = PlayerTr.GetComponent<AdvancedWalkerController>();
        ...
        void Update()
		{
            ...
            {
                smoothX = Mathf.Lerp(smoothX, advancedWalkerController.limitedInputX * 1.5f + oldHorizontalInput * 11, Time.fixedDeltaTime * smoothRot);
                tr.localRotation *= Quaternion.Euler(new Vector3(sinAmountY, sinAmountX, smoothX / 5));
            }


            if (isHide && (characterInput.IsInteractKeyPressed() || characterInput.GetVerticalMovementInput() > 0 
                || isInteractMobile || advancedWalkerController.mobile_Input.MoveInput().z > 0))
            {
                lastNearestInteraction.hideExit(this, advancedWalkerController);
            }
		...
		protected void RotateCamera(float _newHorizontalInput, float _newVerticalInput)
		{
            ...
            if (isHide)
                currentYAngle = Mathf.Clamp(currentYAngle, -7 + cameraHideOffset, 7 + cameraHideOffset);
